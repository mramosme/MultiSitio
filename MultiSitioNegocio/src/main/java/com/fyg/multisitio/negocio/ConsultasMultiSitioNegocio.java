 ...
 */
	...
	public List<Actividad> busquedaActividad(Actividad actividad) {
		//Primero generamos el identificador unico de la transaccion
				String uid = GUIDGenerator.generateGUID(actividad);
				//Mandamos a log el objeto de entrada
				LogHandler.debug(uid, this.getClass(), "Busqueda Actividad - Datos Entrada: " + actividad);
				//Variable de resultado
			    EncabezadoRespuesta respuesta = new EncabezadoRespuesta();
			    respuesta.setUid(uid);
				respuesta.setEstatus(true);
			    ...
			    try {
			    	listaActividad = new ConsultaMultiSitio().consultaActividad(uid,actividad);
				    ...
				    LogHandler.debug(uid, this.getClass(), "Busqueda Actividad - Datos Salida: " + respuesta);
